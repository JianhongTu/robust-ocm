...
# All-in-one script for Kimi-VL benchmarking
# This script:
...
# Model configuration
MODEL_NAME="${MODEL_NAME:-kimi-vl-thinking-2506}"
MODEL_SHORT_NAME="kimivl"  # Short name for output files (e.g., ds, dpsk, internvl, qwenvl, kimivl)
...
# This file is automatically generated by the render CLI
GT_FILE="$DATA_ROOT/longbenchv2_img/text_ground_truth.json"
...
log_section "Kimi-VL Benchmarking Pipeline"
...
micromamba run -n "$CONDA_ENV" python scripts/ocr/kimi_vl_inference.py \
    --input "$DATA_DIR" \
    --output "$PRED_DIR" \
